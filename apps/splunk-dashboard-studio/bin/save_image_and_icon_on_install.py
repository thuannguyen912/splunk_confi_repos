...
KVSTORE_ICON_STATUS_CONF = 'splunk-dashboard-studio/kvstore_icon_status.conf'


def get_kvstore_status(session_key=None):
    logging.info('fetch kvstore status')
...
def wait_kvstore_ready(session_key=None):
    ...
    while kvstore_status == "starting" and retry > 0:
        ...
        if kvstore_status == "ready":
            logging.info(
                'kvstore status is ready, start next step to upload icons')
            return
        retry = retry - 1
...
def read_kvstore_icon_status_conf():
    conf_settings = {}
    try:
        path = get_base_path() + '/' + KVSTORE_ICON_STATUS_CONF
...
if __name__ == '__main__':
    ...
    try:
        ...
        logging.info('dashboard studio version is not matching uploaded version in kvstore icon status conf. '
                     'checking kvstore now ...')

        wait_kvstore_ready(session_key=session_key)

        # update splunk-dashboard-icons kvstore collection
        update_kvstore_collection(
            folder_name='icons',
