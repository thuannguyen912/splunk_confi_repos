...
class AppListHandler(PersistentServerConnectionApplication):
    ...
    def get_compatibility(self, version, compatibility):
        ...
        for item in all_versions:
            ...
            if ('8.0' in splunk_support) or ('8.1' in splunk_support) or ('8.2' in splunk_support):
                quake_support_flag = True
            if version == app_version:
                version_found = True
                if (('8.0' in splunk_support) or ('8.1' in splunk_support) or ('8.2' in splunk_support)) and \
                        len(splunk_support) == 1:
                    return CONST_QUAKE
                elif (('8.0' in splunk_support) or ('8.1' in splunk_support) or ('8.2' in splunk_support)) and \
                        len(splunk_support) > 1:
                    return CONST_DUAL
                else:
                    continue
