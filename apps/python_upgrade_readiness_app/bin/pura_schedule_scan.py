...
class ScheduleScanHandler(PersistentServerConnectionApplication):
    ...
    def get_cron_format(self, schedule_scan_details):
        ...
        try:
            ...
            logging.info("Scheduling scan for hours: {} minutes: {}".format(hours, minutes))
        ...
        try:
            try:
                response, content = sr.simpleRequest(
                    schedule_scan_endpoint_json,
                    sessionKey=self.session_key,
                )
            except Exception as e:
                logging.exception(
                    MESSAGE_EXCEPTION_READ_SCHEDULE_SCAN_DETAILS.format(self.user, self.host)
                )
                return None
    ...
    def post_schedule_scan(self, scan_details):
        ...
        # get the saved schedule scan details
        saved_schedule_scan_details = self.get_saved_schedule_scan_details()
        if saved_schedule_scan_details is None:
            return utils.render_error_json(MESSAGE_ERROR_POST_SCHEDULE_SCAN)
        elif saved_schedule_scan_details:
            # previous saved schedule scan details are present
            try:
                kv_store_key = saved_schedule_scan_details["_key"]
                response, _ = sr.simpleRequest(
                    "{}/{}?output_mode=json".format(schedule_scan_endpoint, kv_store_key),
                    sessionKey=self.session_key,
                ...
                logging.exception(
                    MESSAGE_EXCEPTION_WRITE_SCHEDULE_SCAN_DETAILS.format(self.user, self.host)
                )
                return utils.render_error_json(MESSAGE_ERROR_POST_SCHEDULE_SCAN)
            if response["status"] not in success_codes:
                logging.error(
                    MESSAGE_ERROR_WRITE_SCHEDULE_SCAN_DETAILS.format(self.user, self.host)
                )
                return utils.render_error_json(MESSAGE_ERROR_POST_SCHEDULE_SCAN)
        else:
            try:
                # no previous saved schedule scan details are present
                response, _ = sr.simpleRequest(
                    schedule_scan_endpoint_json,
                    sessionKey=self.session_key,
                    jsonargs=json.dumps(scan_details),
        ...
        try:
            post_args = {"interval": cron_time}
            response, content = sr.simpleRequest(
                "{}?output_mode=json".format(
                    schedule_scan_interval_endpoint.format("$SPLUNK_HOME%252Fetc%252Fapps%252Fpython_upgrade_readiness_app%252Fbin%252Fpura_scan_apps.py")),
                    sessionKey=self.session_key,
                    postargs=post_args,
                    method="POST",
                    raiseAllErrors=True,
        ...
        if response["status"] not in success_codes:
            ...
            return utils.render_error_json(MESSAGE_ERROR_POST_SCHEDULE_SCAN)
        return utils.render_json(MESSAGE_POST_SCHEDULE_SCAN)
    ...
    def get_schedule_scan(self):
        ...
        if schedule_scan_details is None:
            return utils.render_error_json(MESSAGE_ERROR_GET_SCHEDULE_SCAN)
        if not schedule_scan_details:
            schedule_scan_details["schedule_scan_type"] = None
        return utils.render_json(schedule_scan_details)
