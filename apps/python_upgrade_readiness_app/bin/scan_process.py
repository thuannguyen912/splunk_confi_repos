...
class ScanProcess(object):
    ...
    def __init__(self, scan_args):
        ...
        self.telemetry_handler = Telemetry(self.session_key, self.request_body, self.host, self.user)
    ...
    def splunk_python_sdk_check(self, app_name, app_path=None):
        ...
        def add_warning_message(file_path):
            """
    ...
    def parse_sdk_response(self, app, app_meta, report, splunk_sdk_check_report, app_path):
        ...
        try:
            ...
            report['checks'] = self.filter_checks(app[0], app_path, report['checks'])
        ...
        try:
            ...
            for index, app in enumerate(app_type_list):
                ...
                if app[1][0] != CONST_PRIVATE:
                    ...
                    if self.telemetry_handler.telemetry_data:
                        self.telemetry_handler.update_telemetry_data(updated_app_report, app_result, app[0], app[1],
                                                                     True, default=True)
                    self.delete_md5_hash(app[2])
                else:
                    app_path = app[2]
                    current_checksum = self.calculate_app_checksum(app_path)
                    previous_checksum = self.read_md5_hash(app[2])
                    ...
                    if self.telemetry_handler.telemetry_data:
                        self.telemetry_handler.update_telemetry_data(updated_app_report, app_result, app[0], app[1],
                                                                     skip_flag, default=False)

                    if app_result is None:
                        app_result = updated_app_report['summary']['Status']

                    if self.write_hash:
                        self.write_md5_hash(app[2], current_checksum)
            ...
            if self.telemetry_handler.telemetry_data:
                ...
                self.telemetry_handler.telemetry_data['errors'] = error_dict
    ...
    def quake_response(self, app, app_meta, app_status, app_path):
        ...
        """
        dismiss_app_flag, dismiss_app_timestamp = self.get_dismiss_app(app[0], app_path)
        app_report = dict()
    ...
    def filter_checks(self, app_name, app_path, checklist):
        """
        ...
        :return Updated checklist
        """
        dismissed_checks = self.fetch_dismissed_checks(app_name, app_path)
        for _dc in dismissed_checks:
            for checks in checklist:
                if checks['name'] == _dc['check']:
                    for i in checks['messages']:
    ...
    def fetch_dismissed_checks(self, app_name, app_path):
        """
        ...
        """
        query_string = "{{\"$and\":[{{\"app\":\"{}\"}},{{\"host\":\"{}\"}}," \
                       "{{\"user\":\"{}\"}}]}}".format(app_name, self.host, self.user)

        try:
            response, content = sr.simpleRequest('{}'.format(
                dismiss_coll_endpoint), getargs={"output_mode": "json", "query": query_string}, sessionKey=self.session_key)
        except Exception:
        ...
        for entry in content:
            if entry['app'] == app_name and entry['app_path'] == app_path:
                dismiss_list += [entry]
        return dismiss_list

    def check_dismiss_app(self, app_name, app_path, content):
        """
        Returns the value of dismiss_check if the app has been saved for dismiss app in KV store
        :param app_report: application name
        :param content: content json of the result
        """
        for entry in content:
            if entry['app'] == app_name and entry['app_path'] == app_path:
    ...
    def get_dismiss_app(self, app_name, app_path):
        """
        Fetch dismissed apps from kvstore.
        ...
        try:
            response, content = sr.simpleRequest('{}'.format(
                dismiss_app_endpoint), getargs={"output_mode": "json", "query": query_string}, sessionKey=self.session_key)
            content = json.loads(content)
        except Exception:
            logging.exception(MESSAGE_EXCEPTION_FETCHING_DISMISS_APP_ENTRY.format(self.user, self.host, app_name))
            return 0, ""
        if response['status'] not in success_codes:
            logging.error(MESSAGE_ERROR_FETCHING_DISMISS_APP_ENTRY.format(self.user, self.host, app_name))
            return 0, ""
        return self.check_dismiss_app(app_name, app_path, content)

    def calculate_app_checksum(self, app):
        hash = dirhash(app, 'md5')
        logging.info("{}, {} ".format(app, hash))
        return hash

    def read_md5_hash(self, app):
        try:
            data = None
            if os.path.isfile(MD5_HASH_PATH):
    ...
    def delete_md5_hash(self, app):
        try:
            data = {}
            if os.path.isfile(MD5_HASH_PATH):
                with open(MD5_HASH_PATH, 'r+') as f:
                    data = json.load(f)
            data.pop(app, None)
            with open(MD5_HASH_PATH, 'w+') as file_handler:
    ...
    def write_md5_hash(self, app, checksum):
        try:
            data = {}
            if os.path.isfile(MD5_HASH_PATH):
                with open(MD5_HASH_PATH, 'r+') as f:
